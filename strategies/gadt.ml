...
let macd = Data (Float_type (Data.Type.Tacaml (F Tacaml.Indicator.Float.Macd_MACD)))
let macd_signal = Data (Float_type (Data.Type.Tacaml (F Tacaml.Indicator.Float.Macd_MACDSignal)))
let macd_hist = Data (Float_type (Data.Type.Tacaml (F Tacaml.Indicator.Float.Macd_MACDHist)))
let bb_upper = Data (Float_type (Data.Type.Tacaml (F Tacaml.Indicator.Float.UpperBBand)))
let bb_lower = Data (Float_type (Data.Type.Tacaml (F Tacaml.Indicator.Float.LowerBBand)))
let bb_middle = Data (Float_type (Data.Type.Tacaml (F Tacaml.Indicator.Float.MiddleBBand)))
let willr = Data (Float_type (Data.Type.Tacaml (F Tacaml.Indicator.Float.Willr)))
let stoch_k = Data (Float_type (Data.Type.Tacaml (F Tacaml.Indicator.Float.Stoch_SlowK)))
let stoch_d = Data (Float_type (Data.Type.Tacaml (F Tacaml.Indicator.Float.Stoch_SlowD)))

(* Smart constructors for integer indicators - candlestick patterns *)
let hammer = Data (Int_type (Data.Type.Tacaml (I Tacaml.Indicator.Int.CdlHammer)))
let doji = Data (Int_type (Data.Type.Tacaml (I Tacaml.Indicator.Int.CdlDoji)))
let engulfing = Data (Int_type (Data.Type.Tacaml (I Tacaml.Indicator.Int.CdlEngulfing)))
let morning_star = Data (Int_type (Data.Type.Tacaml (I Tacaml.Indicator.Int.CdlMorningStar)))
let evening_star = Data (Int_type (Data.Type.Tacaml (I Tacaml.Indicator.Int.CdlEveningStar)))
let shooting_star = Data (Int_type (Data.Type.Tacaml (I Tacaml.Indicator.Int.CdlShootingStar)))
let hanging_man = Data (Int_type (Data.Type.Tacaml (I Tacaml.Indicator.Int.CdlHangingMan)))
let piercing = Data (Int_type (Data.Type.Tacaml (I Tacaml.Indicator.Int.CdlPiercing)))
let dark_cloud = Data (Int_type (Data.Type.Tacaml (I Tacaml.Indicator.Int.CdlDarkCloudCover)))
...
let rec eval : type a. a expr -> Data.t -> int -> (a, Error.t) result = 
  ...
  | Div (e1, e2) ->
    let* v1 = eval e1 data index in
    let* v2 = eval e2 data index in
    if Float.equal v2 0.0 then
      Error.fatal "Division by zero in GADT.eval"
    else
      Result.return (v1 /. v2)
...
let candlestick_patterns_strategy = {
  ...
  sell_trigger = 
    (engulfing IntLT Int 0) ||.
    (close >. (close *. Float 1.1));
  max_positions = 6;
  position_size = 0.16;
}

(* Helper function to evaluate strategy triggers *)
let eval_strategy_signal strategy_expr state symbol =
  let data = State.get_bars state symbol in
  let current_index = Data.current data in
  match eval strategy_expr data current_index with
  | Ok should_signal -> Result.return should_signal
  | Error e -> Error e
...
let gadt_to_buy_trigger (buy_expr : bool expr) =
  let module Buy_input : Template.Buy_trigger.INPUT = struct
    let pass state symbol =
      eval_strategy_signal buy_expr state symbol
    
    let score _state _symbol =
      Result.return 1.0
...
let gadt_to_sell_trigger (sell_expr : bool expr) =
  let module Sell_impl : Template.Sell_trigger.S = struct
    let make state ~buying_order =
      let symbol = Order.symbol buying_order in
...
let gadt_to_strategy_builder (strategy : strategy) =
  ...
  let module Strategy = Template.Make (val buy_trigger) (val sell_trigger) in
  (module Strategy : Strategy.BUILDER)