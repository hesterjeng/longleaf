...
let dual_sma_rsi =
  (* Dual SMA + RSI *)
  {
    ...
    score = Const (100.0, Float) -. Gadt_fo.Variable.rsi ();
...
let mean_reversion_1min =
...

  {
    ...
    buy_trigger = rsi_2 <. Const (30.0, Float) &&. (last <. bb_lower);
...
(** 1-Minute Mean Reversion Strategy - Optimizable Version
    ...
    This version uses Variables instead of Constants, allowing NLopt to
    optimize:
    ...
    NLopt will run this strategy with different parameter combinations and find
    the values that maximize returns (or Sharpe ratio).

    To optimize: longleaf Backtest MeanReversionOpt --target
    data/jan24_1min.json
...
let mean_reversion_opt =
  ...
  let rsi_period_var = Gadt_fo.var Gadt.Type.Int in
  (* Variable: RSI period (2-14) *)
  let bb_period_var = Gadt_fo.var Gadt.Type.Int in
...

  {
    ...
    buy_trigger = rsi_var <. Const (30.0, Float) &&. (last <. bb_lower_var);
    (* Sell: Return to mean OR overbought OR price hit upper band *)
    sell_trigger =
      last >. bb_middle_var
      ||. (rsi_var >. Const (70.0, Float))
      ||. (last >. bb_upper_var);
    score = Const (100.0, Float) -. rsi_var;
    (* Lower RSI = higher score *)
    max_positions = 10;
    position_size = 0.10;
  }
...
(** MeanReversion_8_27 - Optimized 1-Minute Mean Reversion Strategy
    ...
    This strategy was discovered through NLopt ISRES optimization over 1000
    evaluations on September-October 2025 data. It uses an 8-period RSI and
    27-period Bollinger Bands.

    Optimization Results:
    - Objective value: 105,749.10
    - RSI period: 8 (vs. standard 2 or 14)
    - BB period: 27 (vs. standard 20)
    - Both longer than typical settings, suggesting this catches medium-term
      reversions
...
let mean_reversion_8_27 =
...

  {
    ...
    buy_trigger = rsi_8 <. Const (30.0, Float) &&. (last <. bb_lower_27);
    sell_trigger =
      last >. bb_middle_27
...
(** MeanReversion_8_27_Safe - Production-Ready Version with Risk Controls
    ...
    Same entry/exit logic as MeanReversion_8_27, but adds critical risk
    management:
...
(** MeanReversion_Safe_Opt - Optimizable Version with Fixed Risk Controls
    ...
    ISRES-optimizable version of the safe mean reversion strategy. Indicator
    periods and thresholds are variables; risk management is fixed.

    Variables to optimize (4 total): 1. rsi_period: [5, 30] - RSI lookback
    period (wider to find slow RSI like CosmicCowgirl) 2. bb_period: [15, 60] -
    Bollinger Band period (wider to allow longer bands) 3. rsi_oversold:
    [20.0, 40.0] - RSI buy threshold 4. rsi_overbought: [60.0, 85.0] - RSI sell
    threshold
    ...
    Entry: RSI(var) < var AND Last < BB_lower(var) Exit: Signals OR
    stop-loss(fixed) OR profit(fixed) OR max_hold(fixed) *)
let mean_reversion_safe_opt =
  (* Create variables for optimizable parameters *)
  let rsi_period_var = Gadt_fo.var ~lower:5.0 ~upper:30.0 Gadt.Type.Int in
  (* Variable 1: RSI period *)
  let bb_period_var = Gadt_fo.var ~lower:15.0 ~upper:60.0 Gadt.Type.Int in
  (* Variable 2: BB period *)
  let rsi_oversold_var = Gadt_fo.var ~lower:20.0 ~upper:40.0 Gadt.Type.Float in
  (* Variable 3: RSI oversold threshold *)
  let rsi_overbought_var =
    Gadt_fo.var ~lower:60.0 ~upper:85.0 Gadt.Type.Float
  in
...

  {
    ...
    buy_trigger = rsi_var <. rsi_oversold_var &&. (last <. bb_lower_var);
    sell_trigger =
...
(** RocketReef - Optimized Mean Reversion Strategy
    ...
    This strategy emerged from NLopt optimization achieving an objective value
    of 106,246.32.
    ...
    - This captures mean reversion around RSI midpoint, not traditional
      oversold/overbought
...
let rocket_reef =
...

  {
    ...
    buy_trigger = rsi_53 <. Const (49.961254, Float) &&. (last <. bb_lower_27);
...
let rocket_reef_day_only =
...

  {
    ...
    buy_trigger =
      rsi_53
      <. Const (49.961254, Float)
      &&. (last <. bb_lower_27) &&. safe_to_enter ();
    sell_trigger =
      force_exit_eod () ||. (last >. bb_middle_27)
      ||. (rsi_53 >. Const (51.425345, Float))
      ||. (last >. bb_upper_27) ||. stop_loss 0.02 ||. profit_target 0.05
      ||. max_holding_time 60;
    score = Const (1.0, Float);
    (* Default score *)
    max_positions = 10;
...
(** RocketReef_DayOnly_Opt - Optimizable Intraday Mean Reversion
    (Multi-Position)
    ...
    Variables to optimize (7 total): 1. rsi_period: [40, 60] - RSI lookback
    period 2. rsi_buy_threshold: [20.0, 50.0] - RSI oversold entry level 3.
    rsi_sell_threshold: [50.0, 80.0] - RSI exit level 4. bb_period: [15, 35] -
    Bollinger Band period 5. stop_loss_pct: [0.01, 0.05] - Stop-loss percentage
    (1-5%) 6. profit_target_pct: [0.02, 0.10] - Profit target percentage (2-10%)
    ...
    Multiple positions allow portfolio diversification while maintaining
    strategy focus. *)
let rocket_reef_day_only_opt =
  (* Create variables for optimizable parameters *)
  let rsi_period_var = Gadt_fo.var Gadt.Type.Int in
  (* Variable 1 *)
  let rsi_buy_threshold_var = Gadt_fo.var Gadt.Type.Float in
  (* Variable 2 *)
  let rsi_sell_threshold_var = Gadt_fo.var Gadt.Type.Float in
  (* Variable 3 *)
  let bb_period_var = Gadt_fo.var Gadt.Type.Int in
  (* Variable 4 *)
  let stop_loss_var = Gadt_fo.var Gadt.Type.Float in
  (* Variable 5 *)
  let profit_target_var = Gadt_fo.var Gadt.Type.Float in
  (* Variable 6 *)
  let max_hold_ticks_var = Gadt_fo.var Gadt.Type.Int in
  (* Variable 7 *)
...

  {
    ...
    buy_trigger =
      rsi_var <. rsi_buy_threshold_var &&. (last <. bb_lower_var)
      &&. safe_to_enter ();
    (* Sell: End-of-day exit first, then mean reversion signals and risk management *)
    sell_trigger =
      force_exit_eod () ||. (last >. bb_middle_var)
      ||. (rsi_var >. rsi_sell_threshold_var)
      ||. (last >. bb_upper_var) ||. stop_loss_var_risk
      ||. profit_target_var_risk ||. max_hold_var_risk;
    score = Const (100.0, Float) -. rsi_var;
    (* Lower RSI = higher score *)
    max_positions = 10;
    position_size = 0.10;
...
(** RocketReef_Stoch - Optimizable Stricter Version with Fast Stochastic Filter
    ...
    This builds on RocketReef by adding a Fast Stochastic Oscillator filter to
    create stricter entry conditions. All indicator periods are variables for
    optimization.

    Variables to optimize (5 total): 1. rsi_period: [40, 60] - RSI lookback
    period 2. bb_period: [20, 35] - Bollinger Band period 3. stoch_fastk_period:
    [10, 20] - Stochastic %K period 4. stoch_fastd_period: [2, 5] - Stochastic
    %D smoothing period 5. stoch_threshold: [15.0, 30.0] - Oversold threshold
    for stochastic
...
let rocket_reef_stoch_opt =
  ...
  let bb_period_var = Gadt_fo.var Gadt.Type.Int in
  (* Variable 2: BB period *)
  let stoch_fastk_period_var = Gadt_fo.var Gadt.Type.Int in
  (* Variable 3: Stochastic %K period *)
  let stoch_fastd_period_var = Gadt_fo.var Gadt.Type.Int in
  (* Variable 4: Stochastic %D period *)
  let stoch_threshold_var = Gadt_fo.var Gadt.Type.Float in
...

  {
    ...
    buy_trigger =
      rsi_var
      <. Const (49.961254, Float) (* RSI weakness *)
      &&. (last <. bb_lower_var) (* Price at lower BB *)
      &&. (stoch_fast_k_var <. stoch_threshold_var);
    (* Deep oversold *)
    (* Sell: Mean reversion signals OR risk management exits *)
    sell_trigger =
      (* Mean reversion complete signals *)
...
(** Stochness - Optimized RocketReef_Stoch
    ...
    Found via NLopt ISRES optimization (50000 iterations). Best objective:
    106732.91 (6.73% return).
    ...
    Entry: RSI < 50 AND price < lower BB AND fast stoch < 5.11 Exit: Mean
    reversion OR 2% stop OR 5% target OR 60-tick hold *)
let stochness =
  (* Optimized parameters *)
...

  {
    ...
    sell_trigger =
      ...
      ||. (last >. bb_upper) ||. stop_loss 0.02 ||. profit_target 0.05
      ||. max_holding_time 60;
    score = Const (1.0, Float);
    (* Default score *)
    max_positions = 10;
...
(** StochnessMonster - Optimized RocketReef_Stoch (Better Performance)
    ...
    Found via NLopt ISRES optimization (iteration 49998). Performance: 9.78%
    return with 3352 orders.
...
let stochness_monster =
  ...
  let rsi_period = 11 in
  (* Fast RSI *)
  let bb_period_buy = 92 in
  (* Very long BB for entries *)
  let stoch_fastk_period = 69 in
  (* Very long stochastic K *)
  let stoch_fastd_period = 6 in
  (* Standard D smoothing *)
  let stoch_threshold = 46.74 in
  (* Less strict oversold *)
  let bb_period_sell = 42 in
  (* Medium BB for exits *)
...
(** TrendRider - Optimizable Trend Following Strategy
    ...
    Instead of buying weakness and selling strength, this strategy rides
    momentum.

    Variables to optimize (6 total): 1. fast_ema_period: [8, 20] - Fast EMA
    period for trend detection 2. slow_ema_period: [20, 50] - Slow EMA period
    for trend confirmation 3. adx_period: [10, 20] - ADX period for trend
    strength measurement 4. adx_threshold: [20.0, 30.0] - Minimum ADX for trend
    entry 5. stop_loss_pct: [0.015, 0.035] - Stop loss (1.5-3.5%) 6.
    profit_target_pct: [0.03, 0.08] - Profit target (3-8%)
...
let trend_rider_opt =
  ...
  (* Variable 3 *)
  let adx_threshold = Gadt_fo.var Gadt.Type.Float in
  (* Variable 4 *)
  let stop_loss_var = Gadt_fo.var Gadt.Type.Float in
  (* Variable 5 *)
  let profit_target_var = Gadt_fo.var Gadt.Type.Float in
...

  {
    ...
    sell_trigger =
      ...
      ||. stop_loss_var_risk (* Variable stop *) ||. profit_target_var_risk
      (* Variable target *) ||. max_holding_time 120;
    (* 120 ticks max *)
    score = adx;
    (* Higher ADX = stronger trend = higher score *)
    max_positions = 5;
    (* Fewer positions for trend riding *)
    position_size = 0.20;
    (* 20% per position *)
  }
...
(** SlowGlide - Optimized TrendRider (Subtle Trend Following)
    ...
    The close convergence of EMAs (79 vs 90) suggests that dramatic crossovers
    don't work for trends - instead, subtle shifts in very long-term averages
    signal sustainable momentum.

    Performance: Modest 0.79% in 2 weeks, suggesting trend-following may
    struggle in choppy/mean-reverting markets where your other strategies excel.
*)
let slow_glide =
  (* Optimized parameters *)
...

  {
    ...
    sell_trigger =
      ...
      ||. (macd_main <. macd_signal)
      ||. (last
          <. EntryPrice -. (Const (1.0, Float) *. Const (14.087366, Float)))
      ||. (last
          >. EntryPrice +. (Const (1.0, Float) *. Const (36.540345, Float)))
...
(** QuickSnap - Optimized RocketReef_DayOnly (Fast RSI + Stable Bands)
    ...
    Result from ISRES optimization: 102598.18 (2.6% return over 2 weeks) This
    significantly outperforms the original RocketReef variants.
    ...
    Key Innovation - Fast RSI, Slow Bands: The 11-period RSI paired with
    51-period Bollinger Bands creates a "spring-loaded" effect:
    ...
    Exit Strategy Innovation: Unlike typical mean reversion (exit at RSI 70+),
    QuickSnap exits at RSI 50.81 - just above neutral. This is brilliant:
    ...
    Entry: RSI(11) < 21.18 AND price < lower BB(51) Exit: RSI(11) > 50.81 OR
    price > middle BB(51) OR EOD OR 88 ticks
...
let quick_snap =
...

  {
    ...
    buy_trigger =
      rsi_11
      <. Const (21.178191, Float)
      &&. (last <. bb_lower_51) &&. safe_to_enter ();
    sell_trigger =
      force_exit_eod () ||. (last >. bb_middle_51)
      ||. (rsi_11 >. Const (50.812802, Float))
          (* Exit near neutral - key insight! *)
      ||. (last >. bb_upper_51)
      ||. (last
          <. EntryPrice -. (Const (1.0, Float) *. Const (84.109595, Float)))
      ||. (last
          >. EntryPrice +. (Const (1.0, Float) *. Const (60.904661, Float)))
      ||. max_holding_time 88;
    score = Const (1.0, Float);
    (* Default score *)
    max_positions = 10;
...
(** ProfessionalMeanRev_Opt - Mean Reversion with Distinct Entry/Exit Indicators
    ...
    KEY INSIGHT - ASYMMETRIC INDICATORS: Entry and exit may work best with
    ...
    Variables to optimize (6 total - STRATEGY LOGIC ONLY): 1. entry_rsi_period:
    [5, 50] - RSI period for entry detection 2. entry_rsi_threshold:
    [20.0, 40.0] - RSI oversold entry level 3. entry_bb_period: [30, 80] - BB
    period for entry (can be long/stable) 4. exit_rsi_period: [10, 60] - RSI
    period for exit (can differ from entry) 5. exit_rsi_threshold: [50.0, 75.0]
    \- RSI exit level 6. exit_bb_period: [15, 50] - BB period for exit (can be
    short/responsive)
    ...
    WHY NO PROFIT TARGET? Past optimizations (DeepReef: 82% target, RR1.0: 37%
    target) show that ISRES overfits profit targets to specific large moves in
    backtest data. Mean reversion should exit when the reversion completes
    (price > middle BB, RSI normalizes), not at arbitrary price levels.
    ...
    Note: Upper BB condition removed as redundant - if price > middle BB, the
    mean reversion trade is complete. *)
let professional_mean_rev_opt =
  (* Entry indicators - optimizable *)
  let entry_rsi_period = Gadt_fo.var Gadt.Type.Int in
  (* Var 1 *)
  let entry_rsi_threshold = Gadt_fo.var Gadt.Type.Float in
  (* Var 2 *)
  let entry_bb_period = Gadt_fo.var Gadt.Type.Int in
  (* Var 3 *)

  (* Exit indicators - DISTINCT from entry, also optimizable *)
  let exit_rsi_period = Gadt_fo.var Gadt.Type.Int in
  (* Var 4 *)
  let exit_rsi_threshold = Gadt_fo.var Gadt.Type.Float in
  (* Var 5 *)
  let exit_bb_period = Gadt_fo.var Gadt.Type.Int in
  (* Var 6 *)
...

  {
    ...
    buy_trigger =
      entry_rsi <. entry_rsi_threshold &&. (last <. entry_bb_lower)
      &&. safe_to_enter ();
    ...
    sell_trigger =
      ...
      ||. max_holding_time 90;
    (* 90 ticks (fixed) *)
    score = Const (100.0, Float) -. entry_rsi;
    (* Lower entry RSI = higher score *)
    max_positions = 10;
    position_size = 0.10;
  }
