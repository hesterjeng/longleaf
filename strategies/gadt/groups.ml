...
let correlation_indicators =
  [ correl (* Pearson Correlation: >0.8 strong positive, >0.5 moderate positive, <-0.8 strong negative, <-0.5 moderate negative *) ]
...
let beta_indicators =
  [ beta (* Beta coefficient: >1.2 high volatility, >1.5 very high volatility, <0.8 low volatility, <0.5 very low volatility *) ]
...
let moving_averages_only = [
  dema; ema; kama; ma; sma; tema; trima; wma;
  mama_mama; mama_fama;
]

(* Bollinger Bands family - volatility-based bands *)
let bollinger_bands_only = [
...
let macd_family = [
  ...
  macd_ext_macd; macd_ext_signal; macd_ext_hist;
  apo; ppo; (* Related oscillators *)
]
...
let candlestick_patterns = [
  ...
  cdl_engulfing; cdl_eveningdojistar; cdl_eveningstar; cdl_gap_side_side_white;
...
let mathematical_transforms = [
  ...
  sin; sinh; sqrt; tan; tanh; add; div; mult; sub;
]
...
let hilbert_transform_family = [
  ht_trendline; ht_dc_period; ht_dc_phase; ht_phasor_inphase;
  ht_phasor_quadrature; ht_sine_sine; ht_sine_leadsine; ht_trend_mode;
]
...
let get_list = function
  ...
  | Crossover_pairs -> [] (* Crossover pairs are handled separately since they're tuples *)
...
(* Special function for crossover pairs since they return tuples, not individual indicators *)
let get_crossover_pairs () = crossover_pairs

(* Function to get group size, with special handling for crossover pairs *)
let group_size = function
  | Crossover_pairs -> List.length crossover_pairs
  | group -> List.length (get_list group)
...
(* Function to get group name as string *)
...
module Bounds = struct
  ...
  let get_bounds = function
    ...
    | Price_comparable -> (0.0, Float.max_value) (* Price-based, no upper bound *)
    | Momentum_centered_zero -> (Float.min_value, Float.max_value) (* Unbounded around zero *)
    | Rate_of_change_around_one -> (0.0, 3.0) (* Typically 0.5 to 2.0, allow some buffer *)
    | Rate_of_change_percentage -> (-50.0, 50.0) (* Typically -20% to +20%, allow buffer *)
    | Volume_indicators -> (0.0, Float.max_value) (* Volume-based, positive unbounded *)
    | Volatility_positive -> (0.0, Float.max_value) (* Volatility measures, positive only *)
    | Correlation_indicators -> (-1.0, 1.0)
    | Beta_indicators -> (0.0, 3.0) (* Typically 0.2 to 2.0, allow buffer *)
    | Slope_indicators -> (Float.min_value, Float.max_value) (* Can be any slope value *)
    | Index_indicators -> (0.0, Float.max_value) (* Index positions, non-negative *)
    | Crossover_pairs -> (0.0, 0.0) (* Special case: crossovers don't have direct bounds *)
    | Moving_averages_only -> (0.0, Float.max_value) (* Price-based, no upper bound *)
    | Bollinger_bands_only -> (0.0, Float.max_value) (* Price-based, no upper bound *)
    | MACD_family -> (Float.min_value, Float.max_value) (* Mix of bounded and unbounded *)
    | Stochastic_family -> (0.0, 100.0) (* All stochastic indicators are 0-100 *)
    | Directional_movement_family -> (0.0, 100.0) (* Most are 0-100, with some unbounded *)
    | Candlestick_patterns -> (-100.0, 100.0) (* Typically return -100, 0, or 100 *)
    | Mathematical_transforms -> (Float.min_value, Float.max_value) (* Math functions, unbounded *)
    | Price_aggregations_only -> (0.0, Float.max_value) (* Price-based, positive *)
    | Linear_regression_family -> (Float.min_value, Float.max_value) (* Mixed types *)
    | Hilbert_transform_family -> (Float.min_value, Float.max_value) (* Mixed types *)
  ...
  let get_thresholds = function
    ...
    | Momentum_centered_zero -> 
      [-1.0; -0.5; 0.0; 0.5; 1.0] (* strong bearish; bearish; neutral; bullish; strong bullish *)
    | Rate_of_change_around_one -> 
      [0.95; 0.98; 1.0; 1.02; 1.05] (* strong bearish; bearish; neutral; bullish; strong bullish *)
    | Rate_of_change_percentage -> 
      [-5.0; -2.0; 0.0; 2.0; 5.0] (* strong bearish; bearish; neutral; bullish; strong bullish *)
    | Volume_indicators -> 
      [] (* Volume thresholds are relative to recent averages *)
    | Volatility_positive -> 
      [] (* Volatility thresholds are relative to recent averages *)
    | Correlation_indicators -> 
      [-0.8; -0.5; 0.0; 0.5; 0.8] (* strong negative; moderate negative; neutral; moderate positive; strong positive *)
    | Beta_indicators -> 
      [0.5; 0.8; 1.0; 1.2; 1.5] (* very low; low; market; high; very high volatility *)
    | Slope_indicators -> 
      [-0.5; -0.1; 0.0; 0.1; 0.5] (* strong bearish; bearish; neutral; bullish; strong bullish *)
    | Index_indicators -> 
      [5.0; 10.0; 15.0] (* very recent; recent; stale *)
    | Crossover_pairs -> 
      [] (* Crossover signals are binary - no thresholds needed *)
    | Moving_averages_only -> 
      [] (* Price comparison thresholds depend on actual price data *)
    | Bollinger_bands_only -> 
