...
let always_buy = {
  ...
  sell_trigger = Bool false;  (* Never sell *)
  max_positions = 10;
  position_size = 0.1;
}
...
let never_buy = {
  ...
  sell_trigger = Bool true;  (* Always sell (but nothing to sell) *)
  max_positions = 1;
  position_size = 1.0;
}
...
let extreme_sizing = {
  ...
  max_positions = 1;
  position_size = 1.0;  (* All-in on each trade *)
}
...
let micro_positions = {
  ...
  max_positions = 100;
  position_size = 0.001;  (* 0.1% per position *)
}

(* Strategy 11: High Frequency - Test many trades *)
let high_frequency = {
  name = "High Frequency";
  buy_trigger = (close >. close +. Float 0.01);  (* Price moved up by $0.01 *)
  sell_trigger = (close <. close -. Float 0.01);  (* Price moved down by $0.01 *)
  max_positions = 20;
  position_size = 0.05;
}
...
let math_stress_test = {
  ...
  buy_trigger = 
    (((rsi *. Float 0.5) +. (adx *. Float 0.3)) >. Float 40.0) &&.
    (((bb_upper -. bb_lower) /. bb_middle) >. Float 0.02) &&.
    ((close /. sma) >. Float 1.01);
  sell_trigger = 
    (((rsi *. Float 0.7) -. (adx *. Float 0.2)) <. Float 30.0) ||.
...
let conservative_candlestick = {
  ...
  position_size = 0.15;  (* Larger size per position *)
}
...
let aggressive_candlestick = {
  ...
  position_size = 0.03;  (* Smaller size per position *)
}
