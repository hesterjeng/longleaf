...
module Conv = struct
  ...
  let generate_strikes ~current_price ~range_pct =
    ...
    let round_to_interval price = Float.round (price /. interval) *. interval in
      ...
      if remaining_weeks <= 0 then List.rev acc
        ...
        let next_friday =
          ...
          let days_to_friday =
            ...
            | `Sun -> 5
          in
          let days_to_add = if days_to_friday = 0 then 7 else days_to_friday in
  ...
  let create_contract ~underlying ~expiry ~option_type ~strike =
    let symbol = generate_occ_symbol ~underlying ~expiry ~option_type ~strike in
