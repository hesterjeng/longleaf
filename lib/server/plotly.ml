...
let get_color index = 
  color_palette.(index mod Array.length color_palette)
...
let of_state ?(start = 100) ?end_ (state : 'a Longleaf_state.t) symbol :
  ...
  let result =
    ...
    let* data = Bars.get bars symbol in
    let price_trace = direct_price_trace ~start ~color:(get_color 0) ?end_ data symbol in
    ...
    in
    let* indicators = Result.map_l (fun (i, indicator) -> trace_with_color i indicator) 
      (List.mapi (fun i x -> (i, x)) all_indicators) in
