...
let calculate_order_stats orders =
  let all_orders =
    Vector.fold (fun acc order_list -> order_list @ acc) [] orders
  in
  ...
  in
  let total_volume =
    List.fold_left (fun acc (order : Order.t) -> acc + order.qty) 0 all_orders
  in
  let total_cash_traded =
    List.fold_left
      (fun acc (order : Order.t) ->
        acc +. (order.price *. float_of_int order.qty))
      0.0 all_orders
  in
  let symbols_traded =
    all_orders
