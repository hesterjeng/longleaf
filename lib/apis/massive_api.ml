...
let response_of_yojson_safe x =
  ...
  | _ ->
    ...
    | Ppx_yojson_conv_lib.Yojson_conv.Of_yojson_error (exn, json) ->
      ...
      Result.fail
      @@ `JsonError
           ("Error decoding Massive_api.response: " ^ Printexc.to_string exn)
    | s ->
      Eio.traceln "[massive_api] %a" Yojson.Safe.pp x;
      let e = Printexc.to_string s in
      Eio.traceln "[massive_api] %s" e;
      Result.fail
      @@ `JsonError "Error while decoding json of Massive_api.response")

let massive_client eio_env _sw =
  Cohttp_eio.Client.make ~https:None (Eio.Stdenv.net eio_env)
...
module Make (Config : CONFIG) = struct
  let massive_key =
    ...
    | Some s -> s
    | None ->
      invalid_arg "No massive key when trying to make massive connection"

  let headers = Headers.of_list [ ("Content-Type", "application/json") ]

  (* Build endpoint with apiKey query parameter *)
  let add_api_key uri = Uri.add_query_param' uri ("apiKey", massive_key)

  let get path =
    let endpoint =
      make_url path |> Uri.of_string |> add_api_key |> Uri.to_string
    in
  ...
  module Download = struct
    ...
    let is_regular_hours (timestamp : Ptime.t) : bool =
      ...
      let is_market_hours =
        ...
        && Float.compare minutes_until_close 0.0 > 0
      in
    ...
    let parse_item data i item =
      ...
      set_data_fields data i ~timestamp ~open_:item.o ~high:item.h ~low:item.l
    ...
    let results_to_data ~minimal_allocation results =
      ...
      if filtered_count < original_count then
        Eio.traceln
          "      Filtered %d bars to %d regular hours bars (removed %d \
           extended hours)"
    ...
    let build_endpoint (request : Request.t) symbol =
      ...
      let path =
        Format.sprintf "/v2/aggs/ticker/%s/range/%s/%s/%s/%s" symbol multiplier
          timespan from_date to_date
      in

      Uri.of_string (make_url path) |> add_api_key |> fun u ->
      Uri.add_query_param' u ("adjusted", "true") |> fun u ->
      Uri.add_query_param' u ("sort", "asc") |> fun u ->
      Uri.add_query_param' u ("limit", "50000") |> Uri.to_string

    (* Fetch data for a single instrument with pagination *)
    let rec fetch_instrument_data_paginated ~minimal_allocation
        (request : Request.t) instrument acc_results endpoint =
      let ( let* ) = Result.( let* ) in
      ...
      | "DELAYED" ->
        ...
        if String.equal massive_resp.status "DELAYED" then
          Eio.traceln "    Note: Receiving delayed data for %a" Instrument.pp
            instrument;
        ...
        | Some next_url ->
          ...
          let next_endpoint =
            Uri.of_string next_url |> add_api_key |> Uri.to_string
          in
          Eio.traceln "Pagination: fetching next page (%d results so far)"
            (List.length all_results);
          fetch_instrument_data_paginated ~minimal_allocation request instrument
            all_results next_endpoint
        | None ->
      ...
      | status -> Error.fatal (Format.sprintf "Massive API error: %s" status)

    let fetch_instrument_data ~minimal_allocation (request : Request.t)
        instrument =
    ...
    let generate_market_timeline (start_time : float) (end_time : float)
        (interval_seconds : float) : float list =
      (* Market hours in minutes since midnight ET *)
      let market_open_minutes = (9.0 *. 60.0) +. 30.0 in
      (* 9:30 AM = 570 minutes *)
      let market_close_minutes = 16.0 *. 60.0 in
      (* 4:00 PM = 960 minutes *)
      let interval_minutes = interval_seconds /. 60.0 in

      (* Convert start/end times to Ptime for date extraction *)
      let start_ptime =
        Ptime.of_float_s start_time
        |> Option.get_exn_or "massive_api: invalid start_time"
      in
      let end_ptime =
        Ptime.of_float_s end_time
        |> Option.get_exn_or "massive_api: invalid end_time"
      in
      let (start_year, start_month, start_day), _ =
        Ptime.to_date_time start_ptime
      in
      ...
      let generate_day_bars (year, month, day) =
        ...
        if weekday >= 1 && weekday <= 5 then
          ...
          let ref_ptime =
            Ptime.of_date_time ((year, month, day), ((12, 0, 0), 0))
            |> Option.get_exn_or "massive_api: failed to create reference time"
          in
          ...
          let num_bars =
            int_of_float
              ((market_close_minutes -. market_open_minutes) /. interval_minutes)
            + 1
          in

          List.init num_bars (fun i ->
              let minutes_et =
                market_open_minutes +. (float_of_int i *. interval_minutes)
              in
              let hours_et = int_of_float (minutes_et /. 60.0) in
              let mins_et =
                int_of_float (minutes_et -. (float_of_int hours_et *. 60.0))
              in

              (* Create Ptime treating ET time as if it were UTC *)
              let ptime =
                Ptime.of_date_time
                  ((year, month, day), ((hours_et, mins_et, 0), 0))
                |> Option.get_exn_or
                     "massive_api: failed to create market bar time"
              in
        ...
        else [] (* Weekend - no bars *)
      in

      (* Recursively generate bars for all days in range *)
      let rec generate_days current_date =
        let current_ptime =
          Ptime.of_date current_date
          |> Option.get_exn_or "massive_api: invalid current_date"
        in
    ...
    let align_to_timeline ~minimal_allocation
        (data_list : (Instrument.t * Data.t) list) (interval_seconds : float) :
        ((Instrument.t * Data.t) list, Error.t) result =
      let ( let* ) = Result.( let* ) in

      if List.is_empty data_list then Result.return data_list
      else (
        Eio.traceln
          "Aligning %d symbols to fixed market schedule (9:30-4:00 PM ET)..."
        ...
        let earliest_times =
          List.map
            (fun (_, data) ->
              Data.get data Data.Type.Time
                0 (* First timestamp in this symbol's data *))
            data_list
        in
        let latest_times =
          List.map
            (fun (_, data) ->
              ...
              Data.get data Data.Type.Time (size - 1)
              (* Last timestamp in this symbol's data *))
            data_list
        in

        (* Find the overall earliest and latest timestamps across all symbols *)
        let start_time =
          List.fold_left Float.min (List.hd earliest_times) earliest_times
        in
        let end_time =
          List.fold_left Float.max (List.hd latest_times) latest_times
        in

        (* Generate fixed market timeline *)
        let timeline =
          generate_market_timeline start_time end_time interval_seconds
        in
        ...
        Result.map_l
          (fun (instrument, sparse_data) ->
            if Data.size sparse_data = 0 then (
              Eio.traceln "  WARNING: %a has no data" Instrument.pp instrument;
              Error.fatal
                (Format.asprintf "Symbol %a has no data in regular hours"
              ...
              for aligned_tick = 0 to num_bars - 1 do
                ...
                let target_time =
                  Data.get longest_data Data.Type.Time aligned_tick
                in

                (* Advance sparse_idx past any bars before target_time *)
                let threshold = target_time -. (interval_seconds /. 2.0) in
                while
                  !sparse_idx < sparse_size
                  && Stdlib.( < )
                       (Data.get sparse_data Data.Type.Time !sparse_idx)
                       threshold
                do
                ...
                let source_idx =
                  if !sparse_idx < sparse_size then
                    ...
                    if
                      Stdlib.( < )
                        (abs_float (sparse_time -. target_time))
                        tolerance
                    then (
                      last_valid_idx := Some !sparse_idx;
                      Some !sparse_idx)
                    else !last_valid_idx (* Forward-fill from last valid *)
                  else !last_valid_idx (* Past end of data, forward-fill *)
                in

                (* Copy data from source to aligned timeline *)
                let src_idx =
                  match source_idx with
                  | Some idx -> idx
                  | None ->
                    first_bar_idx (* Back-fill: use first bar if no data yet *)
                in
                Data.set aligned_data Data.Type.Index aligned_tick
                  (float_of_int aligned_tick);
                Data.set aligned_data Data.Type.Time aligned_tick target_time;
                Data.set aligned_data Data.Type.Open aligned_tick
                  (Data.get sparse_data Data.Type.Open src_idx);
                Data.set aligned_data Data.Type.High aligned_tick
                  (Data.get sparse_data Data.Type.High src_idx);
                Data.set aligned_data Data.Type.Low aligned_tick
                  (Data.get sparse_data Data.Type.Low src_idx);
                Data.set aligned_data Data.Type.Close aligned_tick
                  (Data.get sparse_data Data.Type.Close src_idx);
                Data.set aligned_data Data.Type.Last aligned_tick
                  (Data.get sparse_data Data.Type.Close src_idx);
                Data.set aligned_data Data.Type.Volume aligned_tick
                  (Data.get sparse_data Data.Type.Volume src_idx)
              done;
      ...
      let* r =
        ...
        Result.map_l
          (fun request ->
            ...
            let* aligned_res =
              align_to_timeline ~minimal_allocation res interval_seconds
            in
