...
let weekday_of_date year month day =
  let ptime =
    ...
    |> Option.get_exn_or "time.ml: weekday_of_date invalid date"
  in
...
let find_nth_weekday year month target_weekday n =
  ...
  let days_to_first =
    if target_weekday >= first_weekday then target_weekday - first_weekday
...
let is_dst year month day hour =
  let dst_start_day = find_nth_weekday year 3 7 2 in
  (* 2nd Sunday in March *)
  let dst_end_day = find_nth_weekday year 11 7 1 in
  ...
  | 3 ->
    (* March: DST starts on 2nd Sunday at 2 AM *)
  ...
  | 11 ->
    (* November: DST ends on 1st Sunday at 2 AM *)
...
let et_utc_offset_hours (unix_time : float) : float =
  let ptime =
    Ptime.of_float_s unix_time
    |> Option.get_exn_or "time.ml: et_utc_offset_hours invalid timestamp"
  in
...
let time_of_day_et (unix_time : float) : float =
  let ptime =
    Ptime.of_float_s unix_time
    |> Option.get_exn_or "time.ml: time_of_day_et invalid timestamp"
  in
...
let add_days (year, month, day) days_to_add =
  let base_ptime =
    Ptime.of_date (year, month, day)
    |> Option.get_exn_or "time.ml: add_days invalid date"
  in
  let seconds_to_add = days_to_add * 86400 in
  let new_ptime =
    Ptime.add_span base_ptime (Ptime.Span.of_int_s seconds_to_add)
    |> Option.get_exn_or "time.ml: add_days overflow"
  in
