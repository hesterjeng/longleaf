...
let respond_404 ?(cors_origin = None) () =
  respond_text ~cors_origin ~status:`Not_found "404 Not Found"

let respond_500 ?(cors_origin = None) message =
...
let handler env cors_origin static_dir _conn request body =
  ...
  let result =
    ...
    | `OPTIONS, _ -> Result.return @@ respond_text ~cors_origin ~status:`OK ""
    ...
    | `GET, "/" ->
      ...
      | Some dir ->
        ...
        | None ->
          ...
          let json =
            Settings.yojson_of_t Settings.settings |> Yojson.Safe.to_string
          in
    ...
    | `POST, "/set_strategy" ->
      ...
      let* () =
        ...
        | true ->
          ...
          Result.return ()
        | false -> Error.fatal "Could not find strategy in data directory"
      in
      Result.return
      @@ (`Assoc [ ("message", `String "settings.cli_vars.strategy_arg set") ]
         |> Yojson.Safe.to_string |> respond_json ~cors_origin)
    ...
    | `POST, "/set_runtype" ->
      let* body_string = Tools.read_body body in
      let r = Yojson.Safe.from_string body_string |> Core.Runtype.t_of_yojson in
    ...
    | `GET, path ->
      ...
      | Some dir ->
        ...
        let file_path =
          if String.prefix ~pre:"/" path then
...
module Args = struct
  ...
  let port_arg =
    let doc =
      "Port for HTTP server to listen on (default from LONGLEAF_PORT env var \
       or 8080)."
    in
  ...
  let cors_origin_arg =
    let doc =
      "CORS origin for React frontend (default from REACT_PORT env var). Use \
       --no-cors to disable CORS."
    in
    Cmdliner.Arg.(
      value
      & opt (some string) default_cors_origin
      & info [ "cors-origin" ] ~doc)
  ...
  let static_dir_arg =
    let doc =
      "Directory containing static files to serve (e.g., React build \
       directory). If set, serves static files and falls back to index.html \
       for SPA routing."
    in
    Cmdliner.Arg.(
      value
      & opt (some string) default_static_dir
      & info [ "static-dir"; "s" ] ~doc)
end
...
module Cmd = struct
  let run port cors_origin no_cors static_dir =
    ...
    | None -> Eio.traceln "Static file serving: disabled");
  ...
  let top =
    ...
    in
    let doc =
      "Longleaf HTTP server for strategy management and visualization."
    in
