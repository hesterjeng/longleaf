...
let respond_404 () =
  respond_text ~status:`Not_found "404 Not Found"

let respond_500 message =
  respond_text ~status:`Internal_server_error message
...
let handler env _conn request body =
  ...
  | `GET, path when String.starts_with ~prefix:"/data/" path && String.ends_with ~suffix:"/json" path ->
  ...
  | `POST, "/set_cli" ->
    let body_string = Cohttp_eio.Body.to_string body in
    (try
       let r = Yojson.Safe.from_string body_string |> Core.Options.CLI.t_of_yojson in
